...
def main(status, count, date_from, date_to, dag_stream, submitter, offline_mode=False):
    ...
    print(f'sessions reasons not matched:\n')
    for session in ci_test_sessions_stat:
        if session['session_id'] not in sessions_id_list_mssql:
            ...
            if session['failure_reasons'] != 'None' and session['failure_reasons'] != sessions_id_dict_mssql[session['session_id']]['failure_reasons']:
                if session['failure_reasons'].find(',') > 0:
                    wf_session_reasons = set(session['failure_reasons'].split(','))
                else: 
                    wf_session_reasons = session['failure_reasons']
                if sessions_id_dict_mssql[session['session_id']]['failure_reasons']:
                    if sessions_id_dict_mssql[session['session_id']]['failure_reasons'].find(',') > 0:
                        mssql_session_reasons = set(sessions_id_dict_mssql[session['session_id']]['failure_reasons'].split(','))
                    else:
                        mssql_session_reasons = sessions_id_dict_mssql[session['session_id']]['failure_reasons']
                else:
                    mssql_session_reasons = 'None'
                if wf_session_reasons != mssql_session_reasons:
                    mssql_update = f"update [GSE_CI].[dbo].[smoke_ci_sessions] set failure_reasons='{session['failure_reasons']}' where session_id={session['session_id']}"
                    mssql_update_commands += mssql_update + '\n'
                    mssql_update_count += 1
                    print(f"session_id:{session['session_id']}")
                    print(f"        WF:{session['failure_reasons']}")
                    print(f"     MSSQL:{sessions_id_dict_mssql[session['session_id']]['failure_reasons']}\n")
