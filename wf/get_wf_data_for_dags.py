...
def main(status, count, date_from, date_to, dag_stream, submitter, filename, offline_mode=False):
    ...
    cache_file3_name = f'jobs_list_dict_tmp.cache'
    cache_wf = False
    #cache_wf = True
    ...
    if cache_wf and os.path.isfile(cache_file1_name) and os.path.isfile(cache_file2_name):
        ...
        for dag_name in dags_list:
            update_stats(ci_cycle_stat, ci_test_sessions_stat, get_cycles(dag_name, submitter=submitter, status=status, count=count, date_from=date_from, date_to=date_to))
    ...
    for build_name in builds_list:
        print(f'{build_name}')
        pd_results = pd.DataFrame(df_jobs[df_jobs['build_name'] == build_name].groupby(['gtax_instance','platform','submission_type'])['duration'].sum())
        pd_results['duration'] = pd_results['duration'].apply(seconds_to_hms)
        print(df_2_html(pd_results, f'{build_name}_jobs_duration.html'))
        pd_results = pd.DataFrame(df_jobs[df_jobs['build_name'] == build_name].groupby(['gtax_instance','platform'])['duration'].sum())
        pd_results['duration'] = pd_results['duration'].apply(seconds_to_hms)
        print(df_2_html(pd_results, f'{build_name}_jobs_duration_total.html'))
        pd_results = pd.DataFrame(df_jobs[df_jobs['build_name'] == build_name].groupby(['gtax_instance','platform','submission_type'])['total'].sum())
        print(df_2_html(pd_results, f'{build_name}_task_count.html'))

        pd_results = pd.DataFrame(df_jobs[df_jobs['build_name'] == build_name].groupby(['gtax_instance','platform','submission_type']).agg({'total': 'sum'}).groupby(level=['gtax_instance','platform']).apply(lambda x: round(100 * x / float(x.sum()), 2)))
        print(df_2_html(pd_results, f'{build_name}_task_count_percent.html'))

        pd_results = pd.DataFrame(df_jobs[df_jobs['build_name'] == build_name].groupby(['gtax_instance','platform','submission_type']).agg({'duration': 'sum'}).groupby(level=['gtax_instance','platform']).apply(lambda x: round(100 * x / float(x.sum()), 2)))
        print(df_2_html(pd_results, f'{build_name}_task_duration_percent.html'))

        pd_results = pd.DataFrame(df_jobs[df_jobs['build_name'] == build_name].groupby(['gtax_instance','platform'])['total'].sum())
        ...
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['submission_type'] == 'init') & (df_jobs['gtax_instance'] == 'gtax-igk')].groupby(['platform'])['total'].sum())
        ...
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk')].groupby(['platform'])['duration'].sum())
        ...
        #### platform igk min-max time
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk')].groupby(['test_plan_id','test_plan_name','platform'])['started_date'].min())
        start_time_dict = pd_results.to_dict()
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk')].groupby(['test_plan_id','test_plan_name','platform'])['completed_date'].max())
        end_time_dict = pd_results.to_dict()
        print(add_plot(f'{build_name}_igk_tp_platform_GRAPH.html', get_platform_timeline(start_time_dict, end_time_dict), f'igk {build_name}', plot_height=3000, bars_colors='#AAAAAA', task_key='timeline_key'))

        #### platform gcmxd min-max time
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-gcmxd-fm')].groupby(['test_plan_id','test_plan_name','platform'])['started_date'].min())
        start_time_dict = pd_results.to_dict()
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-gcmxd-fm')].groupby(['test_plan_id','test_plan_name','platform'])['completed_date'].max())
        end_time_dict = pd_results.to_dict()
        #print(add_plot(f'{build_name}_gcmxd_tp_platform_GRAPH.html', get_platform_timeline(start_time_dict, end_time_dict), f'gcmxd-fm {build_name}', plot_height=3000, bars_colors='#AAAAAA', task_key='timeline_key'))

        #### DG2 igk min-max time
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk') & (df_jobs['platform'] == 'DG2')].groupby(['test_plan_id','test_plan_name','pool'])['started_date'].min())
        start_time_dict = pd_results.to_dict()
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk') & (df_jobs['platform'] == 'DG2')].groupby(['test_plan_id','test_plan_name','pool'])['completed_date'].max())
        end_time_dict = pd_results.to_dict()
        print(add_plot(f'{build_name}_igk_DG2_GRAPH.html', get_platform_timeline(start_time_dict, end_time_dict), f'jobset_sessions igk DG2 {build_name}', plot_height=1000, bars_colors='#0000FF', task_key='timeline_key'))

        #### DG2 igk min-max time
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk') & (df_jobs['platform'] == 'DG2')].groupby(['test_plan_id','test_plan_name','jobset_id'])['started_date'].min())
        start_time_dict = pd_results.to_dict()
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk') & (df_jobs['platform'] == 'DG2')].groupby(['test_plan_id','test_plan_name','jobset_id'])['completed_date'].max())
        end_time_dict = pd_results.to_dict()
        print(add_plot(f'{build_name}_igk_DG2_jobsets_GRAPH.html', get_platform_timeline(start_time_dict, end_time_dict), f'jobsets igk DG2 {build_name}', plot_height=8000, bars_colors='#0000FF', task_key='timeline_key'))

        #### DG2 igk csq min-max time
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk') & (df_jobs['platform'] == 'DG2') & (df_jobs['submission_type'] == 'init')].groupby(['test_plan_id','test_plan_name','csq'])['started_date'].min())
        start_time_dict = pd_results.to_dict()
        pd_results = pd.DataFrame(df_jobs[(df_jobs['build_name'] == build_name) & (df_jobs['gtax_instance'] == 'gtax-igk') & (df_jobs['platform'] == 'DG2') & (df_jobs['submission_type'] == 'init')].groupby(['test_plan_id','test_plan_name','csq'])['completed_date'].max())
        end_time_dict = pd_results.to_dict()
        print(add_plot(f'{build_name}_igk_DG2_init_csq_GRAPH.html', get_platform_timeline(start_time_dict, end_time_dict), f'jobsets igk DG2 {build_name}', plot_height=1000, bars_colors='#0000FF', task_key='timeline_key'))
    ...
    print(add_plot(f'{ts}cycles_GRAPH.html', ci_cycle_stat, 'master', plot_height=1000, bars_colors='#ff0000'))
    print(add_plot(f'{ts}sessions_GRAPH.html', ci_test_sessions_stat, 'master', plot_height=3000, bars_colors='#0000bb', task_key='test_plan_name_and_build'))


        #print(pd_results.to_dict().keys())
        #df = pd.DataFrame(pd_results,columns=['platform','total'])
        #df.plot(x ='platform', y='total', kind = 'bar')
        #plt.savefig('f{build_name}_task_count_total.png')

    


    #print(df_jobs.groupby('platform'))
    #print('\n\nTotal duration by platform:')
    #print(df_jobs.groupby('platform')['duration'].sum())
    #print('\n\nTotal tasks by platform:')
    #print(df_jobs[df_jobs['submission_type'] == 'init'].groupby('platform')['total'].sum())

    #print('\n\nTotal tasks by platform:')
    #print(df_jobs[df_jobs['submission_type'] == 'init'].groupby('platform')['total'].sum())


    #print(df_jobs.groupby(['platform','submission_type'])['total'].count())

    #print(df_jobs.groupby(['platform','submission_type'])['total'].sum())

    #print(df_jobs[df_jobs['build_name'] == 'gfx-driver-ci-master-8497'].groupby(['platform','submission_type'])['duration'].sum())
...
def get_cycles(dag_id, stream=None, submitter=None, date_from=None, date_to=None, status=None, count=100):
    cycles_list = list()
...
def update_sessions_with_gtax_data(ci_test_sessions_stat, ci_jobs_data, offline_mode=False):
    ...
    for session in ci_test_sessions_stat:
        call_count += 1
        if session['gtax_id']:
            if session['gtax_instance'] != 'gtax-presi-igk':
...
def get_session_jobs_list(session_jobs_data, session):
    ...
    for job_data in session_jobs_data:
        ...
        if 'csq' in job_data.keys():
            ...
            job_dict.update({'csq':'not_found'})
        for key in ['build_name', 'gtax_instance', 'cycle_id', 'session_id', 'test_plan_id', 'test_plan_name', 'environment']:
...
if __name__ == '__main__':
    ...
    ap.add_argument('-f', '-date_from', default=get_date_n_days_ago(4), help='date from format yyyy-mm-dd', required=False)
    ...
    ap.add_argument('-dag', default='CI_DAILY__gfx-driver__master__silicon', help='DAG name default: CI_DAILY__gfx-driver__master__silicon', required=False)
    #ap.add_argument('-dag', default='CI_DAILY__gfx-driver__comp_vulkan,CI_DAILY__gfx-driver__comp_vulkan__vco,CI_DAILY__gfx-driver__master__wstn,CI_DAILY__gfx-driver__comp_igc__cmc,CI_DAILY__gfx-driver__comp_igc__skl_perf_3d,CI_DAILY__gfx-driver__comp_igc__vco,CI_DAILY__ci-neo_master,CI_DAILY__ci-neo_embargo,CI_DAILY__ci-neo_embargo__postSi_perf,CI_DAILY__ci-neo_master__postSi_perf,CI_DAILY__ci-linux_16.7,CI_DAILY__open-linux-driver-ci-dev_media,CI_DAILY__open-linux-driver-ci-dev_media__MSDK,CI_DAILY__gfx-driver__master__MSDK_Linux,CI_DAILY__open-linux-driver-ci-dev_igc,CI_DAILY__open-linux-driver-ci-dev_igc__perf,CI_DAILY__gfx-driver__comp_ogl,CI_DAILY__gfx-driver__comp_ogl__vco,CI_DAILY__open-linux-driver-ci-dev_media__gta_staging_tests,CI_DAILY__gfx-driver__comp_d3d10__vco_3d_perf,CI_DAILY__gfx-driver__comp_d3d10,CI_DAILY__gfx-driver__master__ags_3_master_plan,CI_DAILY__gfx-driver__master__display,CI_DAILY__gfx-driver__master__MDMScanners,CI_DAILY__gfx-driver__master__gdhm_dumps_testing,CI_DAILY__gfx-driver__comp_media__BehaviorTest,CI_DAILY__gfx-driver__comp_media__ContentProtectionOpen,CI_DAILY__agama_embargo,CI_DAILY__agama_ci_prerelease__GF1_SLES_RHEL,CI_DAILY__agama_ci_prerelease__HPCWL,CI_DAILY__agama_ci_prerelease__E2E,CI_DAILY__agama_release_19.4,CI_DAILY__agama_release_20.1,CI_DAILY__agama_release_20.2,CI_DAILY__gfx-driver__master__MSDK_Windows,CI_DAILY__gfx-driver__comp_grl,CI_DAILY__ci-linux_i915__i915,CI_DAILY__ci-linux_i915_prerelease__sles,CI_DAILY__ci-linux_dkms_ubuntu_oem_prerelease,CI_DAILY__ci-neo_embargo__sparse_ATS_CI_SC,CI_DAILY__ci-linux_i915__i915_ATS_CI_SC,CI_DAILY__ci-linux_i915_prerelease__rhel', help='DAG name default: CI_DAILY__gfx-driver__master__silicon', required=False)
    #ap.add_argument('-dag', default='CI_WEEKLY__gfx-driver__master__uber_gft:gfx-driver__master', help='DAG stream default: CI_DAILY__gfx-driver__master__silicon:gfx-driver__master', required=False)
    #ap.add_argument('-dag', default='CI_WEEKLY__gfx-driver__master__wednesday_weekly:gfx-driver__master', help='DAG stream default: CI_DAILY__gfx-driver__master__silicon:gfx-driver__master', required=False)
