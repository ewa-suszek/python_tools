...
def main(status, count, date_from, date_to, dag_stream, submitter, offline_mode=False):
    ...
    #upload_to_mssql('daily_ci_cycles', ci_cycle_stat)

    #print(f'uploading sessions')
    #upload_to_mssql('daily_ci_sessions', ci_test_sessions_stat)
    return 0
...
def update_sessions_with_gtax_data(ci_test_sessions_stat, offline_mode=False):
    ...
    for session in ci_test_sessions_stat:
        ...
        if session['gtax_id']:
            ...
            jobs_full_info = get_gtax_jobs_full_info(session['gtax_instance'], session['gtax_id'], offline_mode=False)
            session_task_dict = get_session_tasks_dict(jobs_full_info)
            session_jobs_dict = get_session_jobs_dict(jobs_full_info)
            gtax_jobs_dict.update(session_jobs_dict)
            #TODO
    ...
    print('\n')
    print(len(gtax_jobs_dict))
    print(gtax_jobs_dict[list(gtax_jobs_dict.keys())[0]])
