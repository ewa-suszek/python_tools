...
def main(status, count, date_from, date_to, dag_stream, submitter, filename, offline_mode=False):
    ...
    #GETING DATA
    cache_file1_name = f'cycles_dags_tmp.cache'
    cache_file2_name = f'sessions_dags_tmp.cache'
    cache_wf = False
    if cache_wf and os.path.isfile(cache_file1_name) and os.path.isfile(cache_file2_name):
        ...
        for dag in dags_list:
            if dag.find(':') > 0:
                ...
                print(f'wrong dag stream format for {dag}. use DAG:stream exp: CI_SMOKE__gfx-driver__master:gfx-driver__master')
