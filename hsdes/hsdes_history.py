...
def get_query_data(query_id,query_name):
    ...
    query_data_fields = set_data_fields_by_subject(query_subject)
    ...
    query_data = get_xml_query_data(query_xml,query_data_fields)
    column_names = query_data[0].keys()

    print(query_data[0])

    
    #bugs_id_list = [ results['data'][i]['id'] for i in range(len(results['data']))]
    #bugs_id_data = []

    #for i in range (len(bugs_id_list)):
    #    print(f'{i+1}/{len(bugs_id_list)} geting data for id={bugs_id_list[i]}')
    #    bugs_id_data.append(get_record_id_data(bugs_id_list[i]))
    ...
    project = {'title': query_name, 'data': query_data, 'column_names': column_names, 'query_title': query_title,'query_link': query_link, 'query_subject': query_subject}
    return project
...
def get_query_info(query_id):
    ...
    query_xml = '{"queryXml": '+json.dumps(results['metadata']['query.query_xml'])+'}'
    query_info = {'query_subject':query_subject, 'query_title':query_title, 'query_xml':query_xml}
    return query_info
...
def set_data_fields_by_subject(query_subject):
    bug_data_fields = 'id%2Ctitle%2Crelease%2Ccomponent%2Cstatus%2Creason%2Cpriority%2Cexposure%2Ctag%2Csubmitted_date%2Csubmitted_by%2Cupdated_date%2Cupdated_by%2Crev%2Ctenant%2Csubject%2Cclosed_date%2Copen_date%2Cclosed_reason'
...
def get_max_open_time(project_data):
    #for bug_data in project_data:
        #print(bug_data['id'])
    #todo search for open bugs for max time
    return None

def get_max_closed_time(project_data):
    #todo search for closed bugs and max time closed-open
    return None

def get_min_closed_time(project_data):
    #todo search for closed bugs and min time closed-open
    return None

def get_average_closed_time(project_data):
    #todo search for closed bugs and count average time closed-open
    return None
