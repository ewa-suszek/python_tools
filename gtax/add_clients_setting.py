...
def main(gtax_url, client_search_string, setting_name, setting_value, is_csq, is_list_mode, filter_offline, remove_setting):
    updated_count = 0
    ...
    for client in clients_dict:
        if remove_setting:
            print(f'removing setting {setting_name} on {client["name"]} ({client["id"]}) [{client["status"]}]')
        else:
            print(f'setting name {setting_name} value {setting_value} on {client["name"]} ({client["id"]}) [{client["status"]}]')
        if is_list_mode:
            print('list mode - no changes on client!')
        else:
            if remove_setting:
                if remove_gtax_setting(gtax_url, client["id"], setting_name):
                    updated_count += 1
                    print('successful')
                else:
                    print('false!! not removed!!')
            else:
                if set_gtax_setting(gtax_url, client["id"], setting_name, setting_value):
                    print('successful')
                    updated_count += 1
                else:
                    print('false!! not set!!')
...
def set_gtax_setting(gtax_url, client_id, setting_name, setting_value):
    url = f'{gtax_url}/api/v1/clients/{client_id}/settings/{setting_name}'
    data = f'"{setting_value}"'
...
def remove_gtax_setting(gtax_url, client_id, setting_name):
    url = f'{gtax_url}/api/v1/clients/{client_id}/settings/{setting_name}'
...
if __name__ == '__main__':
    usage_msg = '''add_clients_setting.exe -s GK-RKL%% -n setting_name -v value
       add_clients_setting.exe -s FM-RKL%% -i gtax-gcmxd-fm -n setting_name -v value -l
       add_clients_setting.exe -csq ('platform' = 'Rocket Lake') -i gtax-gcmxd-fm -n setting_name -v value -offline skip'''
    ...
    ap.add_argument('-n', '-name', help='setting name', required=True)
    ap.add_argument('-v', '-value', help='setting value', required=False)
    ...
    if not (parsed.s or parsed.csq):
        ...
        if not (parsed.v or parsed.rem):
            ...
            if parsed.s:
                main(gtax_url, parsed.s, parsed.n, parsed.v, False, parsed.l, parsed.offline, parsed.rem)
            if parsed.csq:
                main(gtax_url, parsed.csq, parsed.n, parsed.v, True, parsed.l, parsed.offline, parsed.rem)
