...
def main(status, count, date_from, date_to, dag_stream, submitter, filename, offline_mode=False):
    ...
    for build_name in builds_list:
        ...
        #pd_results = pd.DataFrame(df_jobs[df_jobs['build_name'] == build_name].groupby(['gtax_instance','platform','submission_type']).agg({'total': 'sum'}))
        #pd_results_agg = pd_results / pd_results.groupby(level=['gtax_instance','platform']).sum()
        #print(df_2_html(pd_results_agg, f'{build_name}_task_count_percent.html'))
    ...
    print(add_plot('cycles_GRAPH.html', ci_cycle_stat, 'master', plot_height=1000, bars_colors='#ff0000'))
    print(add_plot('sessions_GRAPH.html', ci_test_sessions_stat, 'master', plot_height=3000, bars_colors='#0000bb', task_key='test_plan_name_and_build'))
